    ...
    public:
        ...
        void removeComponent(EntityID entity) {
            ...
            // This is the "swap-and-pop" method. It keeps the component array tightly packed.
            // 1. Get the index of the component to remove
            size_t indexOfRemoved = m_entityToIndexMap[entity];
            size_t indexOfLast = m_components.size() - 1;

            // 2. Move the last element into the spot of the element to remove
            m_components[indexOfRemoved] = std::move(m_components[indexOfLast]);

            // 3. Update the maps to point to the new location of the moved element
            ...
            // 4. Remove the now-duplicate last element and the old map entries
    ...
    struct AnimationComponent {
        ...
        bool isPlaying = true;
        bool loop = true; // Let's add a looping flag
    };
    ...
    private:
        ...
        void collectRenderCommands() 
        {
            ...
            ComponentArray<AnimationComponent>* animationArray = static_cast<ComponentArray<AnimationComponent>*>(components.getComponentArray<AnimationComponent>());
            ...
            for (const auto& [entityID, meshRendererIndex] : meshRendererArray->getEntityToIndexMap()) 
            {
                ...
                if (transform) 
                {
                    const MeshRenderer& meshRenderer = meshRendererArray->getComponentVector()[meshRendererIndex];
                     RenderCommand cmd;
                    cmd.modelMatrix = transform->matrix; // Data from Transform component
                    ...
                    // SAFELY get the AnimationComponent using the EntityID
                    AnimationComponent* animComponent = animationArray->getComponent(entityID);
                    if (animComponent && animComponent->isPlaying && animComponent->animation)
                    {
                        ...
                        glm::mat4 translationMatrix = glm::translate(glm::mat4(1.0f), animComponent->animation->getPosition());
                        glm::mat4 rotationMatrix = glm::mat4_cast( animComponent->animation->getRotation());
                        glm::mat4 scaleMatrix = glm::scale(glm::mat4(1.0f), animComponent->animation->getScale());
                        glm::mat4 finalMatrix = translationMatrix * rotationMatrix * scaleMatrix;
                        cmd.modelMatrix = cmd.modelMatrix * finalMatrix;
                    }
        ...
        void collectLightData() {
            ...
            auto pointLightEntities = static_cast<ComponentArray<PointLightComponent>*>(components.getComponentArray<PointLightComponent>());
            for (const auto& [entityID, meshRendererIndex] : pointLightEntities->getEntityToIndexMap()) {
                auto* light = components.getComponent<PointLightComponent>(entityID);
                if (light) {
            ...
            // --- Collect Spot Lights ---
            auto spotLightEntities = static_cast<ComponentArray<SpotLightComponent>*>(components.getComponentArray<SpotLightComponent>());
            for (const auto& [entityID, meshRendererIndex] : spotLightEntities->getEntityToIndexMap()) {
                auto* light = components.getComponent<SpotLightComponent>(entityID);
                if (light) {
            ...
            auto* dirLightArray = components.getComponentArray<DirectionalLightComponent>();
